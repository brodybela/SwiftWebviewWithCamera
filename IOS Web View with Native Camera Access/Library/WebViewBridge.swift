...
open class WebViewBridge: NSObject {
    ...
    fileprivate var kJsCheckObjectDefinedCommand: String {
        return "typeof WebViewBridge == \'object\';"
    }
    
    fileprivate var kJsFetchMessagesCommand: String {
        return "WebViewBridge._fetchJSMessage();"
    }
    ...
    public class func bridge(_ webView: UIWebView, defaultHandler handler: SWVBHandler?) -> WebViewBridge {
        
        let bridge = WebViewBridge.init(webView: webView)
...
extension WebViewBridge {
    ...
    fileprivate func dispatchMessage(_ msg: SWVBMessage) {
            ...
            let webView = self.webView
        {
            ...
            let jsCommand = "WebViewBridge._handleMessageFromSwift('\(jsonMsg)')"
    ...
    fileprivate func handleMessagesFromJS(_ jsonMessages: String) {
        ...
        for swvbMsg in messages {
            ...
            else {                 let callback:SWVBResponseCallBack = {
...
extension WebViewBridge: UIWebViewDelegate {
...
extension WebViewBridge {
    ...
    fileprivate func loadMinifiedJS() -> String {
        return "(function(){function p(b){if(WebViewBridge._defaultHandler)throw Error(\"WebViewBridge.init called twice\");WebViewBridge._defaultHandler=b;b=d;d=null;for(var a=0;a<b.length;a++)k(b[a])}function q(){var b=JSON.stringify(f);f=[];return b}function r(b){d?d.push(b):k(b)}function t(b,a){l(null,b,a)}function l(b,a,c){b=b?{handlerName:b,data:a}:{data:a};c&&(a=\"cb_\"+u++ +\"_JS_\"+(new Date).getTime(),g[a]=c,b.callbackId=a);f.push(b);e.src=\"swvbscheme://__SWVB_Host_MESSAGE__\"}function v(b,a){m[b]=\na}function k(b){window.setTimeout(function(){var a=JSON.parse(b),c;if(a.responseId){if(c=g[a.responseId])c(a.responseData),delete g[a.responseId]}else{if(a.callbackId){var d=a.callbackId;c=function(a){f.push({responseId:d,responseData:a});e.src=\"swvbscheme://__SWVB_Host_MESSAGE__\"}}var h=WebViewBridge._defaultHandler;a.handlerName&&(h=m[a.handlerName]);if(h)try{h(a.data,c)}catch(w){\"undefined\"!=typeof console&&console.log(\"WebViewBridge: WARNING: javascript handler threw.\",a,w)}else onerror(\"No defaultHandler!\")}})}\nif(!window.WebViewBridge){var e,f=[],d=[],m={},g={},u=1;window.WebViewBridge={init:p,sendDataToSwift:t,registerHandlerForSwift:v,callSwiftHandler:l,_fetchJSMessage:q,_handleMessageFromSwift:r};e=document.createElement(\"iframe\");e.style.display=\"none\";e.src=\"swvbscheme://__SWVB_Host_MESSAGE__\";document.documentElement.appendChild(e);var n=document.createEvent(\"Events\");n.initEvent(\"WebViewBridgeReady\");document.dispatchEvent(n)}})();"
    }
}
